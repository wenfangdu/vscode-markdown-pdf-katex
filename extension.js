...
function activate(context) {
  ...
  var commands = [
    vscode.commands.registerCommand('extension.markdown-pdf.settings', async function () {
      await markdownPdf('settings')
    }),
    vscode.commands.registerCommand('extension.markdown-pdf.pdf', async function () {
      await markdownPdf('pdf')
    }),
    vscode.commands.registerCommand('extension.markdown-pdf.html', async function () {
      await markdownPdf('html')
    }),
    vscode.commands.registerCommand('extension.markdown-pdf.png', async function () {
      await markdownPdf('png')
    }),
    vscode.commands.registerCommand('extension.markdown-pdf.jpeg', async function () {
      await markdownPdf('jpeg')
    }),
    vscode.commands.registerCommand('extension.markdown-pdf.all', async function () {
      await markdownPdf('all')
    }),
  ]
  ...
  try {
    ...
    var plantumlOptions = {
      openMarker:
        ...
        vscode.workspace.getConfiguration('markdown-pdf')['plantumlOpenMarker'] ||
        '@startuml',
      closeMarker:
        matterParts.data.plantumlCloseMarker ||
        vscode.workspace.getConfiguration('markdown-pdf')['plantumlCloseMarker'] ||
        '@enduml',
...
function exportPdf(data, filename, type, uri) {
  ...
  if (!checkPuppeteerBinary()) {
    showErrorMessage(
      'Chromium or Chrome does not exist! \
  ...
  var StatusbarMessageTimeout =
    vscode.workspace.getConfiguration('markdown-pdf')['StatusbarMessageTimeout']
  ...
  return vscode.window.withProgress(
    ...
    async () => {
      try {
        ...
        var options = {
          executablePath:
            vscode.workspace.getConfiguration('markdown-pdf')['executablePath'] ||
        ...
        if (type == 'pdf') {
          ...
          if (
            vscode.workspace.getConfiguration('markdown-pdf', uri)['orientation'] == 'landscape'
          ) {
          ...
          var options = {
            ...
            displayHeaderFooter: vscode.workspace.getConfiguration('markdown-pdf', uri)[
              'displayHeaderFooter'
            ],
            headerTemplate: transformTemplate(
              vscode.workspace.getConfiguration('markdown-pdf', uri)['headerTemplate'] || '',
            ),
            footerTemplate: transformTemplate(
              vscode.workspace.getConfiguration('markdown-pdf', uri)['footerTemplate'] || '',
            ),
            printBackground: vscode.workspace.getConfiguration('markdown-pdf', uri)[
              'printBackground'
            ],
            ...
            margin: {
              ...
              right:
                vscode.workspace.getConfiguration('markdown-pdf', uri)['margin']['right'] || '',
              bottom:
                vscode.workspace.getConfiguration('markdown-pdf', uri)['margin']['bottom'] || '',
        ...
        if (type == 'png' || type == 'jpeg') {
          ...
          var clip_width_option =
            vscode.workspace.getConfiguration('markdown-pdf')['clip']['width'] || null
          var clip_height_option =
            vscode.workspace.getConfiguration('markdown-pdf')['clip']['height'] || null
          var options
          if (
            clip_x_option !== null &&
            clip_y_option !== null &&
  ...
  try {
    ...
    if (path.isAbsolute(outputDirectory)) {
      if (!isExistsDir(outputDirectory)) {
        showErrorMessage(
          `The output directory specified by the markdown-pdf.outputDirectory option does not exist.\
    ...
    var outputDirectoryRelativePathFile =
      vscode.workspace.getConfiguration('markdown-pdf')['outputDirectoryRelativePathFile']
...
function readStyles(uri) {
  try {
    ...
    if (ishighlight) {
      if (highlightStyle) {
        var css =
          vscode.workspace.getConfiguration('markdown-pdf')['highlightStyle'] || 'github.css'
...
function fixHref(resource, href) {
  try {
    ...
    var stylesRelativePathFile =
      vscode.workspace.getConfiguration('markdown-pdf')['stylesRelativePathFile']
    let root = vscode.workspace.getWorkspaceFolder(resource)
...
function installChromium() {
  try {
    ...
    const revision = require(path.join(__dirname, 'node_modules', 'puppeteer-core', 'package.json'))
